...
export default function Hand() {
    ...
    const capture = async () => {
        setIsLoading(true);
        const res = await fetch('https://e5ad-196-134-62-202.ngrok-free.app', {
            method: 'GET',
            headers: {
    ...
    return (
        <main className="flex min-h-screen flex-col items-center space-y-8 p-10 max-w-lg mx-auto">
            ...
            <div className="space-y-1">
                <h1 className="text-3xl font-bold text-center">Realtime</h1>
                <p className=" text-center text-gray-500">
                    Adjust your hand with the glove then capture on each word
                </p>
                <h3 className="text-2xl font-bold text-center text-blue-600">
                    7 Seconds per letter
                </h3>
            </div>
            <div className="w-full">
                ...
                <div className="flex w-full items-center justify-between mt-2">
                    ...
                    <button
                        className="bg-blue-500 text-white px-4 py-2 rounded-md font-bold w-[65%]"
                        onClick={() => {
